...
const currentYear = new Date().getFullYear(); // Ստեղծում ենք ընթացիկ տարվա փոփոխականը և նրան տալիս ենք ընթացիկ ժամանակը new Date(ստեղծում ենք նոր օբյեկտ Date class-ով, այն վերադարձնում է ընթացիկ ժամանակը), getFullYear() վերադարձնում է ընթացիկ տարին
...
year.innerText = currentYear + 1;// սրանով սահմանվում է էկրանի հետևի ֆոնում գտնվող հաջորդ տարվա տարեթիվը՝ year(const year = document.querySelector('#year') վերևում այս ձևով ստեղծել ենք year փոփոխականը, այստեղ օգտագործելու համար).innerText(այս էլեմենտի միջոցով է երևում էկրանին) = currentYear(ընթացիկ տարին է որը հաշվել ենք վերևում) + 1(գումարում ենք 1 որ ստեղծի հաջորդ տարին)
...
function updateCounter() {
	...
	const hoursLeft = Math.floor(diff / 1000 / 60 / 60) % 24;//նույն մեթոդով հաշվում ենք(diff / 1000 / 60 / 60) թե որքան ժամ է մնացել մինչև հուվարի 1-ը, հետո % 24-ով մենք ստանում ենք վերևում հաշվարկած օրերի մնացորդային օրերը վերածածված ժամերի(քանի որ վերևում օրերը ստացվել են 99․3 ու Math.floor-ը կլորացնում է դեպի ներքև ու վերցնում է ամբողջ օրերի թիվը այսինքն 99-ը, դրա համար այդ մնացորդային 0,3 օրը %(մաջիլո) 24-ի միջոցով մենք վերածում ենք ժամերի )
	...
	const minutesLeft = Math.floor(diff / 1000 / 60) % 60;//նույն մեթոդով հաշվում ենք րոպեները
	...
	const secondsLeft = Math.floor(diff / 1000) % 60;//նույն մեթոդով հաշվում ենք վայրկյանները
